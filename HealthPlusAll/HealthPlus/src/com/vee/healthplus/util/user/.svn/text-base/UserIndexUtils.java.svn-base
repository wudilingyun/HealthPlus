...
public class UserIndexUtils {
    ...
    /**
     * 身体质量指数
     * BMI公式:BMI=体重(千克)/身高*身高(公尺,厘米/100);
     * 18.5~24为正常范围
     * 体重指数——男性：	 	体重指数——女性：
     * 过轻——低于20	 	过轻——低于19
     * 正常—— 20-25	 	正常——19-24
    ...
    public static String getUserBMI(Context mContext, HP_User user) {
        float bmi = user.userWeight / ((user.userHeight / 100) * (user.userHeight / 100));
        DecimalFormat df = new DecimalFormat("##0.00");
        return df.format(bmi) + mContext.getResources().getString(R.string.hp_userinfo_finebmi);
    }
    ...
    public static String getUserBMR(Context mContext, HP_User user) {
        float bmr = 0;
        if (user.userSex == 0) {
            bmr = 655F + (9.6F * user.userWeight) + (1.7F * user.userHeight) - (4.7F * user.userAge);
        } else {
            bmr = 66F + (13.7F * user.userWeight) + (5.0F * user.userHeight) - (6.8F * user.userAge);
        }
        DecimalFormat df = new DecimalFormat("##0.00");
        return df.format(bmr) + mContext.getResources().getString(R.string.hp_userinfo_calorie_day);
    }
    ...
    /**
     * 每天需要热量
     * 每天需要热量＝655.096＋9.563×（W）＋1.85×（H）－4.676×（A）
     * W：体重，公斤为单位
     * H：身高，公分为单位
     * A：年龄，岁为单位
     * 如需增重：每天需要热量+500-1000卡路里
     * 如需减重：每天需要热量-500-1000卡路里
     */
    public static String getEveryDayQuality(Context mContext, HP_User user) {
        DecimalFormat df = new DecimalFormat("##0.00");
        return String.valueOf(df.format(655.096 + 9.563 * user.userWeight + 1.85 * user.userHeight - 4.676 * user.userAge)) + mContext.getResources().getString(R.string.hp_userinfo_calorie_day);
    }

    public static String getAddWeight(Context mContext, HP_User user) {
        DecimalFormat df = new DecimalFormat("##0.00");
        return String.valueOf(df.format(655.096 + 9.563 * user.userWeight + 1.85 * user.userHeight - 4.676 * user.userAge + 500 - 1000)) + mContext.getResources().getString(R.string.hp_userinfo_calorie_day);
    }
