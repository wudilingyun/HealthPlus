package com.vee.healthplus.widget.fancychart;
...
import com.vee.healthplus.widget.fancychart.data.AxisValue;
import com.vee.healthplus.widget.fancychart.data.ChartData;
import com.vee.healthplus.widget.fancychart.data.Point;
...
public class FancyChart extends View {
    ...
    private boolean drawVerticalText = true;
    private boolean drawHorizontalLine = false;
    private boolean drawHorizontalText = false;
    private boolean pointClickble = false;
    private TypedArray typeArray;

    public FancyChart(Context context) {
        this(context, null);
    }

    public FancyChart(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.vpiFancyChartStyle);
    }

    public FancyChart(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);

        this.chartData = new ArrayList<ChartData>();
        this.chartStyle = new FancyChartStyle();
        typeArray = context.obtainStyledAttributes(attrs, R.styleable.FancyChart, defStyle, 0);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (chartStyle.drawBackgroundBelowLine()) {
            for (ChartData data : chartData) {
                ...
                for (Point point : points) {
                    path.lineTo(point.canvasX - 2, point.canvasY);
                }
        ...
        paintBorder.setColor(chartStyle.getBorderColor());
        ...
        for (ChartData data : chartData) {
            ...
            for (Point point : points) {
                canvas.drawCircle(point.canvasX, point.canvasY, chartStyle.getPointRadius() + 2, paintBorder);
...
//                                onPointClickListener.onClick(point);
    ...
    private void drawHorizontalLinesAndLegend(Canvas canvas) {
        int canvasHeight = getHeight() - HORIZONTAL_LEGEND_HEIGHT - HORIZONTAL_LEGEND_MARGIN_TOP;
...
//        paint.setColor(typeArray.getColor(R.styleable.FancyChart_lineColor,0));
    ...
    private void drawVerticalLinesAndLegend(Canvas canvas) {
        ...
        for (AxisValue value : xValues) {
            ...
            if (value.title != null) {
                int length = value.title.length();
                if (drawVerticalText)
                    canvas.drawText(value.title, x - length * 6 / 2, getHeight() - chartStyle.getLegendTextSize() / 2 - 15, paintLegend);
            }
        }
    }
