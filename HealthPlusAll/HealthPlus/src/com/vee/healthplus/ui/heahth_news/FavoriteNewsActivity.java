...
public class FavoriteNewsActivity extends Activity implements TaskCallback,
	...
	private class GetFavoriteNewsListTask extends
		...
		protected String doInBackground(String... params) {
			try {
				Log.i("lingyun", "doInBackground");
				list = HP_DBModel.getInstance(mContext).queryUserCollectInfor(
						HP_User.getOnLineUserId(mContext)); // 收藏列表获取函数
				return "ok";
			} catch (Exception e) {
				Log.i("lingyun", "doInBackground Exception");
			}
			return null;
		...
		protected void onPostExecute(String data) {
			Log.i("lingyun", "onPostExecute");
			if (data == null) {
				Toast.makeText(mContext, "收藏列表获取失败", Toast.LENGTH_SHORT).show();
			} else if (data.equals("ok")) {
				if (list != null) {
					Toast.makeText(mContext, "收藏列表获取完成", Toast.LENGTH_SHORT)
							.show();
					adapter.listaddAdapter(list);
					adapter.notifyDataSetChanged();
				} else {
